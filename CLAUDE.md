...
- **Detecting Vue dropdown open state**: The ERP inserts dropdown content dynamically as a child of the trigger container (e.g. `.action-btn-container`) — it is not pre-rendered and hidden. The correct pattern: on `document` `click`, verify the click landed inside the relevant container, then try to find the target element immediately. If it is not there yet, attach a `MutationObserver` to the trigger container with `{ childList: true, subtree: true }` and disconnect as soon as the target is found. No timeout, no attempt counter, no `requestAnimationFrame` loop — the observer is purely event-driven and costs nothing while idle; once the SPA navigates away the detached container is GC'd. **Do not use `requestAnimationFrame`** — it races with Vue's async DOM insertion and will lose. **Do not check `element.offsetParent !== null`** to test visibility at insertion time — the element may not be painted yet. **Do not observe `document.body`** — unnecessarily broad for a container-scoped insertion. Use `element.textContent?.trim().includes(…)` for text matching. See `rental-list-signature-highlight-controller.ts`.
